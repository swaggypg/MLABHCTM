...
def train(args, snapshot_path):
    ...
    #         model1, optimizer1, start_epoch, best_performance1 = util.load_checkpoint(
    #             snapshot_path + "/" + model_checkpoint1, model1, optimizer1
    #         )
    #         optimizer2,best_performance2 = util.load_checkpoint_tf(
    #             snapshot_path + "/" + model_checkpoint2, optimizer2
    #         )
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            origin_image, weak_batch, strong_batch, label_batch, cls_label_batch= (
                sampled_batch["origin_image"],
                sampled_batch["image_weak"],
                sampled_batch["image_strong"],
                sampled_batch["label_aug"],
                sampled_batch["cls_label"]
            )
            origin_image,weak_batch, strong_batch, label_batch,cls_label_batch = (
                origin_image.cuda(),
                weak_batch.cuda(),
                strong_batch.cuda(),
                label_batch.cuda(),
                cls_label_batch.cuda()
            )

               # handle unfavorable cropping
            # non_zero_ratio = torch.count_nonzero(label_batch) / (8 * 224 * 224)
            # if non_zero_ratio <= 0.02:
            #     logging.info("Refreshing policy...")
            #     refresh_policies(db_train, cta)
            #     continue
            
            outputs1 = model1(origin_image)
            outputs_soft1 = torch.softmax(outputs1, dim=1)
            outputs1_weak = model1(weak_batch, cls = True)
            outputs_soft1_weak = torch.softmax(outputs1_weak['output'], dim=1)
            outputs1_strong = model1(strong_batch)
            outputs_soft1_strong = torch.softmax(outputs1_strong, dim=1)

            outputs2 = model2(origin_image)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            outputs2_weak = model2(weak_batch)
            outputs_soft2_weak = torch.softmax(outputs2_weak, dim=1)
            outputs2_strong = model2(strong_batch)
            outputs_soft2_strong = torch.softmax(outputs2_strong, dim=1)


            consistency_weight = get_current_consistency_weight(
                iter_num // 150)
            
            seg_mean = torch.mean(torch.stack([outputs_soft1[args.labeled_bs:],outputs_soft2[args.labeled_bs:]]),dim=0)
            uncertainty = -1.0 * torch.sum(seg_mean * torch.log(seg_mean + 1e-6), dim=1, keepdim=True)
            umc = torch.mean(uncertainty)

            loss1 = 0.5 * (ce_loss(outputs1_weak['output'][:args.labeled_bs], label_batch[:args.labeled_bs].long()) + dice_loss(
                outputs_soft1_weak[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = 0.5 * (ce_loss(outputs2_weak[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + dice_loss(
                outputs_soft2_weak[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))

            pseudo_outputs1 = torch.argmax(
                outputs_soft1_weak[args.labeled_bs:].detach(), dim=1, keepdim=False)
            pseudo_outputs2 = torch.argmax(
                outputs_soft2_weak[args.labeled_bs:].detach(), dim=1, keepdim=False)

            pseudo_supervision1 = dice_loss(
                outputs_soft1_strong[args.labeled_bs:], pseudo_outputs2.unsqueeze(1))
            pseudo_supervision2 = dice_loss(
                outputs_soft2_strong[args.labeled_bs:], pseudo_outputs1.unsqueeze(1))

            model1_loss = loss1 + consistency_weight * (pseudo_supervision1 + umc)
            model2_loss = loss2 + consistency_weight * (pseudo_supervision2 + umc)

            cls_weight = 0.1
            loss_cls = ce_loss(outputs1_weak['cls'], cls_label_batch) if cls_weight > 0. else 0.

            loss = model1_loss + model2_loss + cls_weight * loss_cls
            ...
            lr_ = base_lr * (1.0 - iter_num / max_iterations) ** 0.9 # **优先级高于*
            ...
            if iter_num % 200 == 0:
                image = origin_image[1,0:1,:, :]
                writer.add_image('train/Origin_Image', image, iter_num)
                ...
                outputs = torch.argmax(torch.softmax(
                    outputs1_weak['output'], dim=1), dim=1, keepdim=True)
                writer.add_image('train/model1_Prediction',
                                 outputs[1, ...] * 50, iter_num)
                outputs = torch.argmax(torch.softmax(
                    outputs1_strong, dim=1), dim=1, keepdim=True)
                writer.add_image('train/model1_Prediction',
                                 outputs[1, ...] * 50, iter_num)
                outputs = torch.argmax(torch.softmax(
                    outputs2_weak, dim=1), dim=1, keepdim=True)
                writer.add_image('train/model2_PredictionS',
                                 outputs[1, ...] * 50, iter_num)
                outputs = torch.argmax(torch.softmax(
                    outputs2_strong, dim=1), dim=1, keepdim=True)
                writer.add_image('train/model2_PredictionS',
                                 outputs[1, ...] * 50, iter_num)
...
if __name__ == "__main__":
    ...
    snapshot_path = "./model/{}_{}/{}_30000_umcO_8_fixmatch".format(
