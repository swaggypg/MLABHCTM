...
class UNet(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.cls_head = ClassificationHead(in_channels=params['feature_chns'][-1], classes=4)  #classes并不于数据集的class相等

    def forward(self, x , cls = False):
        feature = self.encoder(x)
        output = self.decoder(feature)
        if cls == True :
            cls = self.cls_head(feature[-1])
            return {'output' : output, 'cls' : cls}
        else: 
            return output
